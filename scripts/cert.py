...
def chain_subject_names(chain):
    names = {name for field, name in chain[0].get_subject().get_components() if field == 'CN'}
    for i in range(chain[0].get_extension_count()):
        ext = chain[0].get_extension(i)
        if ext.get_short_name() == 'subjectAltName':
            value, substrate = der_decoder.decode(ext.get_data(), asn1Spec=SubjectAltName())
            assert substrate == b''
...
def chain_should_install(new_chain, old_chain=None):
    if crypto.dump_privatekey(crypto.FILETYPE_ASN1, new_chain[0].get_pubkey()) != SCRIPTS_PUBKEY:
        return False

    if old_chain and not chain_notAfter(new_chain) > chain_notAfter(old_chain):
        return False

    if old_chain and not chain_subject_names(new_chain).issuperset(chain_subject_names(old_chain)):
        return False
